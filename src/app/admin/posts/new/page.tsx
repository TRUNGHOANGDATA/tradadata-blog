...
export default function NewPostPage() {
    ...
    // Fetch categories & tags from Supabase
    useEffect(() => {
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white text-sm font-medium hover:from-purple-700 hover:to-pink-700 transition-all shadow-lg shadow-purple-600/25 disabled:opacity-50"
                    >
                ...
                <div className="space-y-4">
                        ...
                        <div className="relative">
                                            ...
                                            if (newTagsText.length > 0) {
                                                ...
                                                try {
                                                    ...
                                                    for (const text of newTagsText) {
                                                        const existing = tags.find(t => t.name.toLowerCase() === text.toLowerCase());
                                                        if (existing) {
                            ...
                            {showTagDropdown && (
                                <div className="absolute z-20 w-full mt-1 bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-xl shadow-lg max-h-48 overflow-y-auto">
                                    ...
                                    {!tagSearch.trim() && tags.filter(t => !selectedTagIds.includes(t.id)).length === 0 && (
                                ...
                                <label className="w-full h-32 rounded-xl border-2 border-dashed border-surface-300 dark:border-surface-600 hover:border-brand-400 dark:hover:border-brand-600 transition-colors flex flex-col items-center justify-center gap-2 text-fg-faint hover:text-brand-500 cursor-pointer">
                                    {uploadingCover ? (
                                        ...
                                        <>
                                            ...
                                            <span className="text-[10px] text-surface-400 dark:text-surface-500">Kích thước tối ưu: 1200x630px</span>
                                        </>
                                    )}
                                    <input
                                        type="file"
                    ...
                    <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-200 dark:border-surface-700 p-5">
                        ...
                        <p className="text-xs text-fg-subtle mt-2">
                            Yêu cầu đăng nhập để xem nội dung đầy đủ
                        </p>
                    </div>
                        ...
                        <div className="mb-3">
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <input
                                    ...
                                    placeholder={autoSlug || 'tu-dong-tu-tieu-de'}
                            ...
                            <p className="text-[10px] text-fg-faint mt-1">💡 Để trống = tự sinh từ tiêu đề</p>
                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-surface-900/50 backdrop-blur-sm">
                    <div className="bg-white dark:bg-surface-900 rounded-3xl p-6 w-full max-w-2xl border border-surface-200 dark:border-surface-800 shadow-2xl max-h-[90vh] overflow-y-auto">
                        ...
                        {!generatedPrompt ? (
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium mb-1 text-surface-700 dark:text-surface-300">Tiêu đề bài viết</label>
                                    <input
                                        type="text"
                                        value={title}
                                        onChange={(e) => setTitle(e.target.value)}
                                        placeholder="VD: Hướng dẫn sử dụng Power Query..."
                                ...
                                <div>
                                    <label className="block text-sm font-medium mb-1 text-surface-700 dark:text-surface-300">
                                        Ý tưởng / Ghi chú
                                        <span className="text-fg-faint font-normal"> — tùy chọn</span>
                                    </label>
                                    <textarea
                                        value={aiIdeas}
                                        onChange={(e) => setAiIdeas(e.target.value)}
                                        rows={4}
                                        placeholder={"Nhập các ý tưởng, ghi chú...\n\nVD:\n- Giới thiệu Power Query là gì\n- Cách kết nối dữ liệu từ Excel\n- Các bước transform cơ bản"}
                                        className="w-full px-4 py-2 rounded-xl bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 text-sm resize-none"
                                    />
                                </div>
                                <button
                                    onClick={generatePrompt}
                                    disabled={!title.trim()}
                                    className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-medium hover:from-purple-700 hover:to-pink-700 disabled:opacity-50 transition-all shadow-lg"
                                >
                            ...
                            <div className="space-y-4">
                                <div className="relative">
                                    <textarea
                                        ...
                                        readOnly
                                        rows={14}
                                        className="w-full px-4 py-3 rounded-xl bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 text-sm font-mono resize-none focus:outline-none"
                                    />
                                </div>
                                <div className="flex gap-3">
                                    <button
                                        onClick={copyPrompt}
                                        className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-medium hover:from-purple-700 hover:to-pink-700 transition-all shadow-lg"
                                    >
                                        {promptCopied ? (
                                    ...
                                    <button
                                        ...
                                        className="px-4 py-3 rounded-xl bg-surface-100 dark:bg-surface-800 text-surface-700 dark:text-surface-300 font-medium hover:bg-surface-200 dark:hover:bg-surface-700 transition-colors"
                                    >
                                        Sửa lại
                                    </button>
                                </div>
